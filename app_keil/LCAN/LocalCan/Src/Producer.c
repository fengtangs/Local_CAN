...
#if !STANDARD_MODE
	CAN_Send.TxMessage.StdId = (BID<<6)+i;		//±ê×¼id ½«¶ÎºÅÓëBIDÉèÖÃÎª±ê×¼ID
	CAN_Send.TxMessage.ExtId = 0x00;					//ÍØÕ¹ID,²»ÓÃ
	CAN_Send.TxMessage.RTR = CAN_RTR_DATA; 		//Êı¾İÖ¡						  
	CAN_Send.TxMessage.IDE = CAN_ID_STD; 			//±íÃ÷Îª±ê×¼Ö¡	
	uint32_t index = Send_Package->Dict_SegmentIndex+ ContentsDictionary[i].StartIndex ;
	CAN_Send.TxMessage.DLC =Data_Length[Dictionary_Data[index].data_type]; 	//Êı¾İ¶Î³¤¶È		
	CAN_Send.Mailbox = CAN_TX_MAILBOX0;
	for(i=1;i<CAN_Send.TxMessage.DLC;i++)		//forÑ­»·ËÙ¶È£¿£¿£¿£¿											//½«Êı¾İ´æÖÁData·¢ËÍ½á¹¹Ìå				  
	{
		CAN_Send.Data[i] = Send_Package->Data[i-1];
	}
	//½«index·ÅÖÁ·¢ËÍ½á¹¹Ìå
...
#else 
	CAN_Send.TxMessage.StdId = 0x00;		//±ê×¼id ½«¶ÎÄÚË÷ÒıÉèÖÃÎª±ê×¼ID,
	CAN_Send.TxMessage.ExtId = (SegmentIndex<<21)+BID+(i<<13);					//ÍØÕ¹ID,¸ß6Î»´æ·Å¶ÎºÅ£¬µÍ5Î»Î»BID
	CAN_Send.TxMessage.RTR = CAN_RTR_DATA; 		//Êı¾İÖ¡						  
	CAN_Send.TxMessage.IDE = CAN_ID_EXT; 			//±íÃ÷ÎªÍØÕ¹Ö¡	
	uint32_t index = Send_Package->Dict_SegmentIndex+ ContentsDictionary[i].StartIndex ;
	CAN_Send.TxMessage.DLC =Data_Length[Dictionary_Data[index].data_type]-1; 	//Êı¾İ¶Î³¤¶È
	if(Dictionary_Data[index].data_type == TYPE_DOUBLE)			//ÌØÊâ´¦Àí
		CAN_Send.TxMessage.DLC = 0x8;
	CAN_Send.Mailbox = CAN_TX_MAILBOX0;
	for(i=0;i<CAN_Send.TxMessage.DLC;i++)													//½«Êı¾İ´æÖÁData·¢ËÍ½á¹¹Ìå				  
	{
		CAN_Send.Data[i] = Send_Package->Data[i];
	}
#endif
		
	
	if(HAL_CAN_AddTxMessage(&hcan, &CAN_Send.TxMessage,CAN_Send.Data, &CAN_Send.Mailbox) != HAL_OK)
	{
		status = 1;
...
static uint32_t Send_MessagesQueue(CAN_Send_package_typedef *Send_Package)
{
 ...
 while( HAL_FDCAN_GetTxFifoFreeLevel(&hfdcan1) == 0 )
  ...
  if(HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1,&CAN_Send.TxMessage,CAN_Send.Data)!=HAL_OK)
	{
		status = 1;
