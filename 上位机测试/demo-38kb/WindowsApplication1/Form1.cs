...
namespace WindowsApplication1
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            if (m_active==1 && m_bOpen == 1 && button_StartCAN.Text == "复位CAN")
            {
                if(FIFO1.Count!=0)
                {
        ...
        private void Active_FID_Click(object sender, EventArgs e)
        {
            int num = 1;
            VCI_CAN_OBJ[] sendob = new VCI_CAN_OBJ[num];//sendobj.Init();
            sendob[0].SendType = (byte)0x0;
            ...
            sendob[0].ExternFlag = (byte)0x0;
            sendob[0].TimeFlag = (byte)0x0;
            sendob[0].ID = 0x00000000;
            sendob[0].DataLen = (byte)0x8;
            sendob[0].Data = new byte[8];
            sendob[0].Data[0] = (byte)0x01;
            sendob[0].Data[1] = 0x01;
            sendob[0].Data[2] = (byte)0xFF;
            sendob[0].Data[3] = (byte)0x6D;
            sendob[0].Data[4] = 0x06;
            sendob[0].Data[5] = (byte)0x4E;
            sendob[0].Data[6] = (byte)0x4D;
            sendob[0].Data[7] = 0x37;
            listBox_Info.Items.Add("激活命令已发出");
            listBox_Info.SelectedIndex = listBox_Info.Items.Count - 1;
            uint res = Device.VCI_Transmit(4, 0, 0, sendob, (uint)num);
            if (res == 0)
            {
                MessageBox.Show("激活失败", "错误", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            m_active = 1;
